...
from IMLearn.desent_methods.modules import LogisticModule, RegularizedModule, L1, L2, sigmoid
from IMLearn.metrics import misclassification_error

R_MODEL_MAP: Dict[str, Union[None, BaseModule]] = {'none': None,
                                                   'l1': L1(),
                                                   'l2': L2()}
...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        """
        r_model: Union[None, BaseModule] = R_MODEL_MAP[self.penalty_]

        d_x_train: int = X.shape[1]
        init_coefs_: np.ndarray = np.random.multivariate_normal(mean=np.zeros(shape=(d_x_train,)),
                                                                 cov=np.eye(d_x_train)) / np.sqrt(d_x_train)
        init_coefs_ = np.reshape(init_coefs_, (len(init_coefs_), 1))
        logistic_model: BaseModule = RegularizedModule(fidelity_module=LogisticModule(),
                                                       regularization_module=r_model,
                                                       lam=self.lam_,
                                                       include_intercept=self.include_intercept_,
                                                       weights=init_coefs_)

        self.coefs_ = self.solver_.fit(f=logistic_model, X=X, y=y)
    ...
    def _predict(self, X: np.ndarray) -> np.ndarray:
        ...
        prob_x: np.ndarray = self.predict_proba(X)
        return np.where(prob_x >= self.alpha_, 1, 0)
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        return sigmoid(np.dot(X, self.coefs_))
